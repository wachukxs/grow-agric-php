...
class Farmer {
    ...
    public function getFarmerWithPasswordByEmail($_email)
    {
        try {
            // Create query, we need the password from this query
    ...
    public function updateFinanceApplicationSeenReceipts($financeapplicationid, $farmerid, $timeread)
    {
        try {
            // Create query
            ...
            `finance_application_statuses`.`seen_by_farmer` = true,
            `finance_application_statuses`.`time_seen_by_farmer` = CURRENT_TIMESTAMP()
            WHERE
            `finance_application_statuses`.`finance_application_id` = :fin_app_id
            ';
            ...
            // Ensure safe data
            $faid = htmlspecialchars(strip_tags($financeapplicationid));
            $fid = htmlspecialchars(strip_tags($farmerid));

            // DELIBRATE TODO: maybe use the $timeread
            $date1 = new DateTime($timeread); // Seems this isn't doing timezone conversion and is not accurate
            $tr = htmlspecialchars(strip_tags($date1->format('Y-m-d H:i:s')));

            // Bind parameters to prepared stmt
            $stmt->bindParam(':fin_app_id', $faid);

            // Execute query statement
            if ($stmt->execute()) {
                file_put_contents('php://stderr', print_r('Executed updateFinanceApplicationSeenReceipts() query' . "\n", TRUE));
                return true;
            } else {
                file_put_contents('php://stderr', print_r('Failed to Execute updateFinanceApplicationSeenReceipts() query' . "\n", TRUE));
                return false;
            }
        } catch (\Throwable $err) {
            // throw $err; $err->getMessage()
            file_put_contents('php://stderr', print_r('Farmer.php->updateReadReceipts error: ' . $err->getMessage() . "\n", TRUE));
            return false;
        }
    }
